...
local fallback_plugin_path = data_dir .. "/plugins/pdf_to_epub_receiver.koplugin"
...
local GITHUB_URL = "https://github.com/quby1845/pdf-to-epub"
local GITHUB_URL_DISPLAY = "github.com/quby1845/pdf-to-epub"
...
function LocalSend:showAbout()
    local ConfirmBox = require("ui/widget/confirmbox")
    local description = plugin_meta.description or _("Receive books from PDF to EPUB OCR over the local network.")
    local arch = self:getDeviceArch() or _("unknown")
    local text = T(_("PDF to EPUB Receiver\n\nVersion: %1\nArchitecture: %2\n\n%3\n\nUses the open LocalSend protocol.\n\n%4"), PLUGIN_VERSION, arch, description, GITHUB_URL_DISPLAY)
...
function LocalSend:addToMainMenu(menu_items)
    ...
    if RECOVERY_MODE then
        menu_items.localsend = {
            text = _("PDF to EPUB Receiver (Recovery Mode)"),
    ...
    menu_items.localsend = {
        text_func = function()
            if REINSTALL_REQUIRED then
                return _("PDF to EPUB Receiver (⚠ Reinstall Required)")
            end
            if self._cached_stopping then
                return _("PDF to EPUB Receiver (stopping...)")
            end
            if self._cached_running then
                if self._cached_transfer_count > 0 then
                    return T(_("PDF to EPUB Receiver (%1 received)"), self._cached_transfer_count)
                end
                return _("PDF to EPUB Receiver (running)")
            end
            if self.update_available_tag ~= "" then
                return _("PDF to EPUB Receiver (update available)")
            end
            return _("PDF to EPUB Receiver")
        end,
        sorting_hint = "network",
